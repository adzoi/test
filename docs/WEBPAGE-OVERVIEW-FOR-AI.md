...
- **Tech:** Static HTML + CSS + vanilla JavaScript. No build step, no framework, no backend. No `<base href>`; all links are **document-relative** so the site works when opened via **file://** and when served from a web root.
- **Purpose:** Marketing site + enquiry/quote forms + two client-side pricing calculators. Forms are intended to submit to a **Google Sheet** for processing by an **AI booking agent** (integration not implemented; forms use `action="#"`).
...
├── quote-estimate.html           # Standalone quote calculator (service + area + location + extras)
...
│   ├── pricing-calculator.css   # Injected calculator on each service page (inside .service-pricing)
│   └── quote-calculator.css     # Quote-estimate page (standalone calculator form + location combobox)
├── js/
...
│   ├── pricing-data.js          # PRICING_SERVICES (16 services, basePrice, inputs with multiplier/add)
│   ├── pricing-calculator.js    # Injected calculator per service page (detect service from filename, render selects, min–max range, Book a Visit, Send Estimate → localStorage)
│   ├── pricing-calculator-standalone.js  # Quote-estimate page: 16 services, area/size, Dublin locations (searchable), extras (×1.2, ×1.5), 90–110% range
...
- **From project root** (e.g. `contact.html`, `404.html`, `quote-estimate.html`):
...
- **From a service page** (e.g. `services/paving-driveways/resin-bound.html`):
  - Assets: `../../css/main.css`, `../../css/service-page.css`, `../../css/pricing-calculator.css`, `../../js/main.js`, `../../js/service-page.js`, `../../js/pricing-data.js`, `../../js/pricing-calculator.js`.
...
### 6.4 quote-estimate.html (Standalone Quote Calculator)
- **Hero:** “Quote Estimate Calculator”, subline about rough estimate.
- **Calculator:** Service dropdown (16 options, filled by JS), Area/Measurement (number input for m² or linear m, or Small/Medium/Large for design/planting/flower beds/features), **Location** (searchable autocomplete combobox — all Dublin areas, ~100+), checkboxes “Ground preparation (×1.2)” and “Premium materials (×1.5)”. Result: “Estimated cost for [SERVICE] in [LOCATION] ([AREA]): €MIN – €MAX”. Buttons: “Get Exact Quote” → `contact.html#form`. Logic in `pricing-calculator-standalone.js`: base price per service (per m², per linear m, or S/M/L fixed), apply extras, then 90–110% range. Location required; validation messages shown if missing.

### 6.5 services/index.html (Services Hub)
...
### 6.6 Service detail pages (16 total)
...
- **Sections in order:**  
  ...
  6. **Pricing** — Title, disclaimer, three pricing cards (e.g. Small/Medium/Large), **injected pricing calculator** below the cards (see §8).  
...
- **Scripts:** `main.js`, `service-page.js`, `pricing-data.js`, `pricing-calculator.js`. **CSS:** `main.css`, `service-page.css`, `pricing-calculator.css`.

### 6.7 about/index.html
- Hero “35 Years of Craft & Care.”, company story, values, team/placeholders, location, CTA. Uses `about.css`, `about.js`.

### 6.8 gallery/index.html
...
## 8. Pricing Calculators (Two Systems)

### 8.1 Per-service calculator (on each of the 16 service pages)
- **Data:** `js/pricing-data.js` — global `PRICING_SERVICES`. Keys = page filename without `.html` (e.g. `resin-bound`, `landscaping-design`). Each service: `displayName`, `basePrice`, `rangePercent`, `inputs[]` (each with `id`, `label`, `type: "select"`, `options` — each option has `label` and either `multiplier` or `add`).
- **Logic:** `js/pricing-calculator.js`. Detects service from `location.pathname` (filename). Injects a block inside `.service-pricing` (after `.service-pricing__grid`): title “Get a rough estimate”, form of selects from `inputs`, “Calculate Estimate” button, result area, disclaimer “This is a rough estimate. Final pricing is confirmed after a site visit.” On submit: compute price from base × multipliers + adds, then min = price×(1−rangePercent), max = price×(1+rangePercent); show “Estimated range: €X – €Y” and buttons “Book a Visit” (→ `../../contact.html`) and “Send Estimate” (writes to `localStorage` key `leaf2leaf_pricing_estimate` with service, min, max, selected, timestamp).
- **Styling:** `css/pricing-calculator.css` (tokens from main, labels mono, values body).

### 8.2 Standalone quote calculator (quote-estimate.html)
- **Data + logic:** `js/pricing-calculator-standalone.js`. Services array with `id`, `name`, `type` (`area_m2` | `linear_m` | `size`), `rate` (number or `{ small, medium, large }`). Dublin locations array (~100+), each “Area County Dublin”. Extras: ground prep ×1.2, premium materials ×1.5. Range 90–110% of total.
- **UI:** Service dropdown, Area (number) or Size (Small/Medium/Large) depending on service, **Location** = searchable combobox (input + hidden input + listbox; filter on type, select sets hidden value; keyboard support). Checkboxes for extras. Result text includes location; “Get Exact Quote” → `contact.html#form`. Location required for calculation.
- **Styling:** `css/quote-calculator.css`.

---

## 9. JavaScript Behaviour Summary
...
- **pricing-calculator.js:** Only runs when `PRICING_SERVICES` and `.service-pricing` exist; injects calculator, handles form submit and result.
- **pricing-calculator-standalone.js:** Populates service/location dropdowns, toggles area vs size field, runs calculation on change, validates location.
...
## 11. How Another AI Should Work With This
...
- **Editing pricing:** Service-page calculator: `pricing-data.js` (add/change service or options) and `pricing-calculator.js` (logic). Standalone calculator: `pricing-calculator-standalone.js` (services, locations, formula).
